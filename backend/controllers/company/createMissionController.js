...
const createMission = async (data) => {
    return new Promise((resolve, reject) => {
        ...
        let query = `SELECT DISTINCT * FROM space_mission s
                    WHERE s.name = ? AND ;`;    // checks if the given name is already taken
        db.query(query, [name], (err, result) => {
            if (err) {
                ...
                db.query(query, [companyId, budget], (err, result) => {
                    ...
                    VALUES (?, ?, CURDATE(), ?, ?, ?, ?, ?, ?);`;
                        db.query(query, [name, location, start_date, end_date, description, budget, companyId, important_notes], (err, result) => {
                            if (err) {
                                reject(err);
                            }
                            else {
